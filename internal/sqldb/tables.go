...
const faviconTableName = "favicon"
...
func InitScanTable() {
	...
	// Clear table if config is true
	if config.GetDBEnv().ClearDBTable {
		err := ClearTable("scan")
		if err != nil {
			log.Error("Fail to Clear table")
		}
	}
}
func InitStaticTables() {

	err := createFavIconHashTable()
	if err != nil {
		panic(err)
	}
	err = createPortTable()
	...
	var faviconCountKey int
	faviconCountRow := QueryRow(DB(), "SELECT COALESCE(COUNT(Hash),0) as count from "+faviconTableName)

	err = faviconCountRow.Scan(&faviconCountKey)
	if err != nil {
		log.Error(err)
	}

	var portCountKey int
	portCountRow := QueryRow(DB(), "SELECT COALESCE(COUNT(Port),0) as count from "+portTableName)

	err = portCountRow.Scan(&portCountKey)
	if err != nil {
		log.Error(err)
	}

	var favIconHashMap map[int32]string
	getFavIconHashList(&favIconHashMap)

	var portMap map[int32]string
	getPortList(&portMap)

	if len(favIconHashMap) != faviconCountKey {
		err = ClearTable(faviconTableName)
		if err != nil {
			log.Fatal(err)
		}

		err = fillTable(faviconTableName, &favIconHashMap)
		if err != nil {
			log.Fatal(err)
		} else {
			log.Info("Table " + faviconTableName + " created with success")
		}
	}

	if len(portMap) != portCountKey {
		err = ClearTable(portTableName)
		if err != nil {
			log.Fatal(err)
		}

		err = fillTable(portTableName, &portMap)
		if err != nil {
			log.Fatal(err)
		} else {
			log.Info("Table " + portTableName + " created with success")
		}
...
func fillTable(tableName string, m *map[int32]string) error {
...
func createScanTable() error {
	_, err := Exec(DB(), "CREATE TABLE IF NOT EXISTS scan"+
		`(
...
func createFavIconHashTable() error {
	_, err := Exec(DB(), "CREATE TABLE IF NOT EXISTS "+faviconTableName+
		`(
		Hash                    INT,
		Service					TINYTEXT,
		PRIMARY KEY (Hash)
...
func createPortTable() error {
	_, err := Exec(DB(), "CREATE TABLE IF NOT EXISTS "+portTableName+
		`(
		Port                    SMALLINT UNSIGNED,
		Service					TINYTEXT,
		PRIMARY KEY (Port)
...
// Source https://github.com/sansatart/scrapts/blob/master/shodan-favicon-hashes.csv
func getFavIconHashList(favIconHashMap *map[int32]string) {
	*favIconHashMap = map[int32]string{
		81586312:    "Jenkins",
		...
		-1240222446: "Zhejiang Uniview | UNV IP camera / NVR DVR",
		...
		1103599349:  "Untangle",
		224536051:   "Shenzhen coship electronics co.,ltd",
		...
		400100893:   "? (DVR)",
		639408214:   "(Blank) (paiza.cloud)",
		-1329446759: "SyncroB - LoRa",
		1787406232:  "Home Assistant",
		341195026:   "NETSurveillance",
		-2076244488: "Draytek",
		611241354:   "Barix Streaming ",
		167837543:   "Nextcloud",
		-782274321:  "Synology DiskStation Manager (DSM)",
	}
}

//Source https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.csv
func getPortList(portMap *map[int32]string) {
